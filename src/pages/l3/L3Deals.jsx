...
import { CheckCircle, XCircle, AlertTriangle, Clock, Zap, Search, Eye, X } from 'lucide-react';
...
export default function L3Deals() {
  ...
  if (search) deals = deals.filter(d =>
    ...
    d.lead?.company?.toLowerCase().includes(search.toLowerCase())
  );

  const handleClose = async (won) => {
    await closeDeal.mutateAsync({ id: closeModal._id, won, notes: closeNote, actorId: user?._id });
    setCloseModal(null); setCloseNote('');
    if (selectedDeal?._id === closeModal._id) setSelectedDeal(null);
  };
  ...
  const columns = [
    ...
    { key: 'value', label: 'Value', sortable: true, render: v => <span style={{ fontWeight: 700, color: 'var(--success)', fontSize: 'var(--text-sm)' }}>{formatCurrency(v)}</span> },
    {
      ...
      render: v => <Badge variant={v === DEAL_STAGE.READY_TO_CLOSE ? 'success' : v === DEAL_STAGE.CLOSED_WON ? 'success' : v === DEAL_STAGE.CLOSED_LOST ? 'danger' : 'muted'}>
    ...
    { key: 'risk_level',   label: 'Risk',   render: v => <RiskBadge risk={v} /> },
    {
      ...
      render: v => v ? <span style={{ fontSize: 'var(--text-xs)', color: new Date(v) < new Date() ? 'var(--danger-light)' : 'var(--text-muted)' }}>{formatDate(v)}</span> : '—',
    },
    {
      key: '_id', label: 'Alert',
      render: (_, row) => {
      ...
      render: (_, row) => {
        ...
        return (
          <div style={{ display: 'flex', gap: 'var(--space-2)' }}>
            {row.lead?._id && (
              <Button size="sm" variant="ghost" icon={Eye} onClick={e => { e.stopPropagation(); openLead(row.lead._id); }}>Lead</Button>
            )}
            {!isClosed && (
              <>
  ...
  return (
    <div className="animate-fade-in">
      ...
      <Table columns={columns} data={deals} loading={isLoading}
        onRowClick={row => setSelectedDeal(row)}
        emptyMessage="No deals match your filters" />

      {/* Deal Detail Side Panel */}
      {selectedDeal && (
        <div style={{ position: 'fixed', top: 0, right: 0, bottom: 0, width: 400, background: 'var(--bg-surface)', borderLeft: '1px solid var(--border)', zIndex: 200, overflowY: 'auto', padding: 'var(--space-6)', animation: 'slideInRight 0.2s ease' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 'var(--space-5)' }}>
            <h3 style={{ fontSize: 'var(--text-lg)', fontWeight: 700 }}>{selectedDeal.lead?.name}</h3>
            <button onClick={() => setSelectedDeal(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--text-muted)', display: 'flex' }}>
              <X size={20} />
            </button>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
            ...
            {selectedDeal.proposals?.length > 0 && (
              <div>
                ...
                {selectedDeal.proposals.map(p => (
                  <div key={p._id} style={{ display: 'flex', justifyContent: 'space-between', padding: 'var(--space-2) var(--space-3)', background: 'var(--bg-surface-2)', borderRadius: 'var(--radius)', marginBottom: 4 }}>
            ...
            {![DEAL_STAGE.CLOSED_WON, DEAL_STAGE.CLOSED_LOST].includes(selectedDeal.stage) && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-2)' }}>
                ...
                {selectedDeal.lead?._id && (
                  <Button variant="secondary" icon={Eye} style={{ width: '100%' }} onClick={() => { openLead(selectedDeal.lead._id); setSelectedDeal(null); }}>
                    View Lead Sheet
                  </Button>
                )}
              </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-4)' }}>
          ...
          <div className="form-group">
            ...
            <textarea value={closeNote} onChange={e => setCloseNote(e.target.value)}
              placeholder={closeModal?._lost ? 'Why was this deal lost? (competitor, budget, timing…)' : 'Final notes, next steps…'}
              rows={3} />
          </div>
        </div>
      </Modal>
